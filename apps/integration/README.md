...
This package is a Vitest integration suite. Its scenarios use the configured MongoDB database and verify behavior with assertions; they do not print demo output or modify application data outside the integration collections.

```sh
# Run every scenario
pnpm --filter @mongorm/integration test

# Run one feature file
pnpm --filter @mongorm/integration test tests/virtual.test.ts

# Pick a scenario by its title
pnpm --filter @mongorm/integration test -t "negative"
```

Configure `MONGODB_URI` and `MONGODB_DATABASE` in this package's `.env` before running the tests. Use a development database: scenarios clear and repopulate their dedicated collections between runs.
